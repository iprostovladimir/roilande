    ...
    function setSpollerAction(e) {
      ...
      if (el.closest("[data-spoller]")) {
        ...
        if (!spollersBlock.querySelectorAll("._slide").length) {
          ...
          if (spollerTitle) {
            const activeText = "Скрыть описание";
            const inactiveText = "Смотреть описание";

            if (spollerTitle.classList.contains("_spoller-active")) {
              spollerTitle.innerText = activeText;
            } else {
              spollerTitle.innerText = inactiveText;
            }
          }
        }
        e.preventDefault();
...
export function duplicateSpollers() {
  const spollersArray = document.querySelectorAll("[data-spollers01]");
  if (spollersArray.length > 0) {
    // Получение обычных слойлеров
    const spollersRegular = Array.from(spollersArray).filter(function (
      item,
      ...
      return !item.dataset.spollers01.split(",")[0];
    });
    // Инициализация обычных слойлеров
    if (spollersRegular.length) {
      initSpollers(spollersRegular);
    ...
    // Получение слойлеров с медиа запросами
    let mdQueriesArray = dataMediaQueries(spollersArray, "spollers01");
    if (mdQueriesArray && mdQueriesArray.length) {
      mdQueriesArray.forEach((mdQueriesItem) => {
        // Событие
        mdQueriesItem.matchMedia.addEventListener("change", function () {
    ...
    // Работа с контентом
    function initSpollerBody(spollersBlock, hideSpollerBody = true) {
      let spollerTitles = spollersBlock.querySelectorAll("[data-spoller01]");
      if (spollerTitles.length) {
        spollerTitles = Array.from(spollerTitles).filter(
          (item) => item.closest("[data-spollers01]") === spollersBlock
        );
        spollerTitles.forEach((spollerTitle) => {
          if (hideSpollerBody) {
            spollerTitle.removeAttribute("tabindex");
    ...
    function setSpollerAction(e) {
      const el = e.target;
      if (el.closest("[data-spoller01]")) {
        const spollerTitle = el.closest("[data-spoller01]");
        const spollersBlock = spollerTitle.closest("[data-spollers01]");
        const oneSpoller = spollersBlock.hasAttribute("data-one-spoller")
          ? true
          : false;
        if (!spollersBlock.querySelectorAll("._slide").length) {
    ...
    function hideSpollersBody(spollersBlock) {
      const spollerActiveTitle = spollersBlock.querySelector(
        "[data-spoller01]._spoller-active"
      );
      if (spollerActiveTitle) {
        spollerActiveTitle.classList.remove("_spoller-active");
...
export function sidebarUnit() {
  let iconMenu01 = document.querySelector(".icon-menu01");
  if (iconMenu01) {
    iconMenu01.addEventListener("click", function (e) {
      if (bodyLockStatus) {
        bodyLockToggle();
        document.documentElement.classList.toggle("menu-open01");
      }
    });
  }
}
export function menuOpen01() {
  bodyLock();
  document.documentElement.classList.add("menu-open01");
}
export function menuClose01() {
  bodyUnlock();
  document.documentElement.classList.remove("menu-open01");
}
